...
Решим задачу уровня сложности hard с picoCTF, хотя на мой лично взгляд данной задаче скорее подходит уровень сложности Medium. Приступим к решению.
...
Здесь сравнивается возвращенное значение strlen и на этом основании выбирается следующий переход, который является проверкой совпадает введенный ключ с тем, что сгенерирован \
...
На данном этапе мы приблизительно поняли, как работает предложенная программа. На следующем шаге отладим программу и уже сможем узнать точные значения регистров 
и получить флаг.
...
Вводим строку, перепрыгиваем на следующую точку останова и нажатием клавиши F7 переходим в функцию sub_1209.
...
Сгенерированные MD5-хэши действительно записались в буферы. А блок со смещениями, о котором мы говорили ранее, это строка с неполным флагом, в которую добавляются символы из 
сгенерированных MD5-хэшей.
